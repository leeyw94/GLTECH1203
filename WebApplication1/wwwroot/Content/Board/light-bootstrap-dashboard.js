...
lbd = {
    ...
    checkNavbarImage: function () {
        $navbar = $('.navbar');
        image_src = $navbar.data('image');

        if (image_src !== undefined) {
            navbar_container = '<div class="navbar-background" style="background-image: url(' + image_src + ') "/>';
            $navbar.append(navbar_container);
        } else if (mobile_menu_initialized == true) {
            // reset all the additions that we made for the sidebar wrapper only if the screen is bigger than 991px
            $navbar_wrapper.find('.navbar-form').remove();
            $navbar_wrapper.find('.nav-mobile-menu').remove();
